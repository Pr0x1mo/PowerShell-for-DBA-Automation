# Define server instance and database
$ServerInstance = "<server_name>.database.windows.net"
$DatabaseName = "<database_name>"
$Username = "<username>"
$Password = "<password>"  # Make sure to securely store and retrieve passwords in a real environment
...
# Check if the Azure SQL Database instance is running
try {
    ...
    $InstanceCheckResult = Invoke-Sqlcmd -ServerInstance $ServerInstance -Database $DatabaseName -Username $Username -Password $Password -Query $InstanceCheckQuery
...
-- Check for running SQL Server Agent jobs
...
WHERE type = 'L'
GROUP BY database_name, DATEPART(hour, backup_start_date)
ORDER BY HourOfDay;

-- Check transaction log generated per hour
...
SELECT t.name AS TableName, resource_type, request_mode, request_status
FROM sys.dm_tran_locks l
JOIN sys.tables t ON l.resource_associated_entity_id = t.object_id
WHERE resource_type = 'OBJECT';

-- Check high resource-consuming SQL queries
...
"@
    ...
    $OtherChecksResult = Invoke-Sqlcmd -ServerInstance $ServerInstance -Database $DatabaseName -Username $Username -Password $Password -Query $OtherChecksQuery
...
Invoke-Sqlcmd -ServerInstance $ServerInstance -Database $DatabaseName -Username $Username -Password $Password -Query $RebuildIndexesQuery

# Perform a regular backup task
$BackupQuery = "BACKUP DATABASE [$DatabaseName] TO DISK = N'C:\Backups\$DatabaseName.bak';"
Invoke-Sqlcmd -ServerInstance $ServerInstance -Database $DatabaseName -Username $Username -Password $Password -Query $BackupQuery
